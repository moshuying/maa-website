...
```jsonc
{
    "TaskName" : {                          // Task name
        ...
        "replace_full": false               // optional, whether to replace the whole string in "ocrReplace"
                                            // replace the matched substring only when set to false
                                            // mainly used for overseas servers where extra prefix/suffix symbols accidentally introduced in ocr can be ignored
                                            // in most situations, this and fullMatch should not be set to true at the same time, otherwise fullMatch would be meaningless
                                            // default false if not filled
